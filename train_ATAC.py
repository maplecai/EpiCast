...
def main(config):
    ...
    # train_index_1, valid_index_1, test_index_1 = utils.split_dataset(index_list_1, train_valid_test_ratio=[0.8, 0.1, 0.1])
    train_dataset_1 = utils.init_obj(datasets, config['train_dataset_1'])
    valid_dataset_1 = utils.init_obj(datasets, config['valid_dataset_1'])
    ...
    # total_dataset_2 = utils.init_obj(datasets, config['train_dataset_2'])
    # index_list_2 = np.arange(len(total_dataset_2))
    # train_index_2, valid_index_2, test_index_2 = utils.split_dataset(index_list_1, train_valid_test_ratio=[0.8, 0.1, 0.1])
    # train_dataset_2 = utils.init_obj(datasets, config['train_dataset_2'], selected_index=train_index_2)
    # valid_dataset_2 = utils.init_obj(datasets, config['valid_dataset_2'], selected_index=valid_index_2)
    ...
    if config.get('load_saved_model', False) is True:
        ...
        for name, param in model.named_parameters():
            if name in config.get('freeze_parameters_list', []):  # 冻结fc2层的参数
    ...
    logger.info(f'start training, num_epochs = {num_epochs}, batch_size = {batch_size}')
    ...
    for epoch in range(num_epochs):
        ...
        if epoch != 0:
            ...
            logger.debug(f'y_true = {y_true[:100]}')
            logger.debug(f'y_pred = {y_pred[:100]}')
        ...
        if (epoch % num_valid_epochs == 0):
            with torch.no_grad():
                ...
                # y_true = []
                # y_pred = []
                # for batch_idx, (x, y) in enumerate(valid_data_loader_2):
                #     x = x.to(device)
                #     y = y.to(device)
                #     pred = model(x)
                #     loss = loss_func(pred[:, [0]], y)

                #     valid_loss += loss.item() / valid_steps
                #     y_true.append(y.cpu().detach())
                #     y_pred.append(pred.cpu().detach())
                    
                # # logger.debug(f'valid_2, epoch = {epoch:3}, valid_loss = {valid_loss:.6f}')
                # y_true = torch.cat(y_true)
                # y_pred = torch.cat(y_pred)

                # for metric_func in metric_func_list:
                #     score = metric_func(y_true, y_pred)
                #     logger.info(f'output_1_valid_2, epoch = {epoch:3}, valid_{type(metric_func).__name__} = {score:.6f}')
                #     score = metric_func(y_true, y_pred[:, [1]])
                #     logger.info(f'output_2_valid_2, epoch = {epoch:3}, valid_{type(metric_func).__name__} = {score:.6f}')
                ...
                # logger.info(f'save model at {checkpoint_path}')
