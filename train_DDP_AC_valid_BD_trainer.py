...
class Trainer_DDP:
    def __init__(self, config):
        ...
        self.train_dataset_2 = utils.init_obj(datasets, config['train_dataset_2'], selected_index=train_index_1)
        self.valid_dataset_2 = utils.init_obj(datasets, config['valid_dataset_2'], selected_index=valid_index_1)
        self.train_sampler_2 = DistributedSampler(self.train_dataset_2)
        self.valid_sampler_2 = DistributedSampler(self.valid_dataset_2)
        self.train_data_loader_2 = utils.init_obj(torch.utils.data, config['data_loader'], dataset=self.train_dataset_2, sampler=self.train_sampler_2, shuffle=False)
        self.valid_data_loader_2 = utils.init_obj(torch.utils.data, config['data_loader'], dataset=self.valid_dataset_2, sampler=self.valid_sampler_2, shuffle=False)
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            ...
            if (epoch == 0):
                ...
                self.valid_epoch(-1, self.valid_data_loader_2, self.valid_sampler_2)

            self.train_epoch(epoch, self.train_data_loader_1, self.train_sampler_1)
            self.train_epoch(epoch, self.train_data_loader_2, self.train_sampler_2)
            ...
            if (epoch % num_valid_epochs == 0):
                ...
                self.valid_epoch(epoch, self.valid_data_loader_2, self.valid_sampler_2)
