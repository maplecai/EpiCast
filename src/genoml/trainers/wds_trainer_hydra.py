...
class WdsTrainerHydra:
    def __init__(self, cfg):
        ...
        if not self.distributed:
            ...
            if cfg['device'] == 'auto':
                ...
                # 例如 ["cuda:0", "cuda:1", ...]
                self.device = cfg['device'][self.local_rank]
            else:
        ...
        self.train_dataset = instantiate(cfg.train_dataset)
        self.valid_dataset = instantiate(cfg.valid_dataset)
        self.train_loader = instantiate(cfg.train_loader, dataset=self.train_dataset)
        self.valid_loader = instantiate(cfg.valid_loader, dataset=self.valid_dataset)
        ...
        self.loss_func = instantiate(cfg.loss_func)
        self.lr_scheduler = instantiate(cfg.lr_scheduler, self.optimizer)
        self.early_stopper = instantiate(
            cfg.early_stopper,
            saved_root_dir=cfg['saved_root_dir'], 
            trace_func=self.log,)
        
        self.metrics_df = pd.DataFrame(columns=['mse', 'r2', 'pearson'])

        metrics_dict = {name: instantiate(mcfg) for name, mcfg in cfg.metrics.items.items()}
        self.metrics = MetricCollection(metrics_dict).to(self.device)
    ...
    def _setup_optimizer(self):
        ...
        if not groups:
            ...
            self.optimizer = instantiate(cfg.optimizer, params=params)
            return

        compiled = [(g, [re.compile(p) for p in (getattr(g, "name_patterns", []) or [])]) for g in groups]
        ...
        for n, p in self.model.named_parameters():
            if not p.requires_grad:
                continue
            hit = False
            for i, (g, pats) in enumerate(compiled):  # first match wins
                if pats and any(r.search(n) for r in pats):
                    buckets[i].append(p); hit = True; break
            if not hit:
                rest.append(p)

        param_groups = []
        for g, ps in zip(groups, buckets):
            if not ps:
                continue
            d = {"params": ps}
            for k in ("lr", "weight_decay"):          # 按需加更多超参
                if hasattr(g, k):
                    d[k] = float(getattr(g, k))
            param_groups.append(d)

        if rest:
            param_groups.append({"params": rest})     # 兜底走 cfg.optimizer 默认 lr/wd

        self.optimizer = instantiate(cfg.optimizer, params=param_groups)
    ...
    def _setup_model(self):
        ...
        if cfg.get('load_saved_model', False) == True:
        ...
        if cfg.get("freeze_parameters", False):
            self.log("freeze parameters")
            patterns = cfg.get("freeze_patterns", [])  # 改名更明确
    ...
    def train(self):
        ...
        epochs_per_valid = cfg['epochs_per_valid']
        ...
        for epoch in range(max_epochs):
            ...
            if ((epoch+1) % epochs_per_valid == 0):
                ...
                if (self.local_rank == 0) and (self.early_stopper is not None):
                    self.early_stopper.check(self.metrics_df.loc[self.epoch, 'pearson'])
    ...
    def train_epoch(self):
        steps_per_log = self.cfg.get('steps_per_log', 0)
        train_steps = 0
        train_loss = 0
    ...
    def valid_epoch(self):
        ...
        #     valid_loss = valid_loss / self.cfg['world_size']
        ...
        if self.local_rank == 0:
            for name, val in self.results.items():
                ...
                self.metrics_df.loc[self.epoch, name] = score
    ...
    def test(self):
        ...
        save_file_path = os.path.join(self.cfg['saved_root_dir'], f'test_pred.npy')
    ...
    def save_checkpoint(self, epoch, step, filename="checkpoint.pt"):
        ...
        save_path = os.path.join(self.cfg['saved_root_dir'], filename)
    ...
    def load_checkpoint(self, filename="checkpoint.pt", load_optimizer=True, load_lr_scheduler=True):
        ...
        load_path = os.path.join(self.cfg['saved_root_dir'], filename)
