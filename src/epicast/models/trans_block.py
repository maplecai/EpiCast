...
class SelfAttention(nn.Module):
    def __init__(self, d_embed, n_heads, dropout_rate=0.1, use_position_embedding=True):
        ...
        if d_embed % n_heads != 0:
            raise ValueError(f"d_embed ({d_embed}) must be divisible by n_heads ({n_heads})")
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor = None):
        ...
        q = self.q_linear(x).reshape(interim_shape).transpose(1, 2)
        k = self.k_linear(x).reshape(interim_shape).transpose(1, 2)
        v = self.v_linear(x).reshape(interim_shape).transpose(1, 2)
...
class TransBlock(nn.Module):
    def __init__(self, d_embed, n_heads, d_mlp, dropout_rate=0.1, use_position_embedding=True):
        super().__init__()
        self.d_embed = d_embed
        self.n_heads = n_heads
        self.d_mlp = d_mlp
...
class TransEncoder(nn.Module):
    def __init__(
        ...
        trans_n_heads: int,
        trans_d_mlp: int,
        trans_dropout_rate: float = 0.1,
    ):
        super().__init__()

        layers = OrderedDict()
        for i in range(num_trans_blocks):
            layers[f"trans_block_{i}"] = TransBlock(
