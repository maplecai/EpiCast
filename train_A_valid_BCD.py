    ...
    for epoch in range(num_epochs):
        ...
        if epoch != 0:
            ...
            for metric_func in metric_func_list:
                score = metric_func(y_pred[:, [0]], y_true)
                logger.info(f'train_1_output_1, {epoch = :3}, train_{type(metric_func).__name__} = {score:.6f}')
        ...
        if (epoch % num_valid_epochs == 0):
            with torch.no_grad():
                ...
                for metric_func in metric_func_list:
                    score = metric_func(y_pred[:, [0]], y_true)
                    logger.info(f'valid_1_output_1, epoch = {epoch:3}, valid_{type(metric_func).__name__} = {score:.6f}')
                ...
                for metric_func in metric_func_list:
                    score = metric_func(y_pred[:, [0]], y_true)
                    logger.info(f'valid_2_output_1, epoch = {epoch:3}, valid_{type(metric_func).__name__} = {score:.6f}')
                    score = metric_func(y_pred[:, [1]], y_true)
                    logger.info(f'valid_2_output_2, epoch = {epoch:3}, valid_{type(metric_func).__name__} = {score:.6f}')
