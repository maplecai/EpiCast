"""fig4B / fig4D / fig4E, mean +- SD variant.

Identical to fig4bde_vef_partial_correlation.py except that the five cell-type values are
summarized by their mean with a one-SD error bar instead of by a box. The two exist side by
side so the style can be chosen on the figure; whichever wins, the other script and its
PDFs get deleted. This one writes `..._mean.pdf` and no legend, since the legend of the
main script serves both.
...
correlation_panels = [
    ("absolute", "fig4b_vef_partial_correlation_mean.pdf"),
    ("residual", "fig4d_vef_residual_partial_correlation_mean.pdf"),
]
...
beta_name = "fig4e_vef_regression_beta_mean.pdf"
