...
from gosai_eval import evaluate_full_model, evaluate_one_model
...
def main():
    ...
    for split in splits:
        for model_name, pred_path in dnase_pred_paths.items():
            ...
            all_model_results.append(
                evaluate_full_model(model_name, pred_df, true_df, cell_types, masks, split, metric_fn_map)
            )

        for model_name, pred_paths in loo_pred_paths.items():
            all_model_results.append(
                evaluate_one_model(model_name, pred_paths, true_df, cell_types, masks, split, metric_fn_map)
            )

    output_dir = "analyze_gosai/results/loo_correlation"
    os.makedirs(output_dir, exist_ok=True)
    combined_df = pd.concat(all_model_results, axis=0, ignore_index=True)

    combined_path = os.path.join(output_dir, "all_models_loo_correlation.tsv")
    combined_df.to_csv(combined_path, sep="\t", index=False)
    print("saved:", combined_path)

    summary_df = combined_df[combined_df["cell_type"] == "loo_mean"].copy()
    summary_path = os.path.join(output_dir, "summary_loo_mean_correlation.tsv")
    ...
    for split in splits:
        for metric_name in metric_fn_map:
            ...
            wide_path = os.path.join(output_dir, f"leave_one_out_{split}_{metric_name}.tsv")
