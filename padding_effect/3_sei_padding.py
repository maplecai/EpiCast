...
if __name__ == '__main__':
    ...
    for cropped_length in [4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096]:
        for seed in range(5):
            set_seed(seed)
            output_path = f'padding_effect/outputs/Sei_pred_crop_{cropped_length}_random_{seed}.npy'
            ...
            test_data_loader = DataLoader(dataset, batch_size=128, shuffle=False, num_workers=1)
            pred = get_pred(model, test_data_loader, device)
            np.save(output_path, pred)
    ...
    for cropped_length in [4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096]:
        for seed in range(5):
            set_seed(seed)
            output_path = f'padding_effect/outputs/Sei_pred_crop_{cropped_length}_genome_{seed}.npy'
